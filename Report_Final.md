...
header-includes:
  ...
  - \pagestyle{fancy}
  - \fancyhf{}
  - \fancyhead[L]{MScT DSAIB X-HEC 2025-27 M1}
  - \fancyhead[R]{Ecole Polytechnique}
  - \fancyfoot[L]{Korouhanba/Bhavesh}
  - \fancyfoot[C]{\thepage}
  - \fancyfoot[R]{APM\_50443\_EP DBMS Project 2026}
...
{\LARGE \textbf{\'Ecole Polytechnique}}\\[0.4cm]
{\large MScT Data Science and AI for Business X-HEC 2025-27}
...
{\LARGE \textit{Implementation on PySpark Using DataFrames/RDD}}\\[0.4cm]

\rule{\linewidth}{0.5pt}

\vspace{1.0cm}

{\large APM\_50443\_EP Database Management}\\[0.3cm]
{\large Final Course Project M1}\\[0.3cm]
{\large \textbf{Completed Under:} Prof. Dario Colazzo}\\[0.3cm]
{\large \textbf{Submitted On:} April 3, 2026}

\vspace{1.5cm}

\begin{tabular}{c}
{\Large \textbf{Bhavesh CHAUHAN}} \\[0.3cm]
{\Large \textbf{Korouhanba Khuman LAIKHURAM}} \\[0.3cm]
\end{tabular}
...
\noindent\textbf{Please Note:} We used Latex and Pandoc in VSCode on a .md file for creating this report, primarily because of the need to include equations explaining the algorithm as in the paper, the need for using unique mathematical symbols in several places to explain the algorithm and for the facility to update our report's results in real-time as we improved the algorithm.

\noindent\textbf{Usage of AI:} We used the help of AI for drafting this report in markdown, especially for writing complex mathematical equations and notations required throughout and complex formatting like code boxes, headers, footers, diagrams and tabulation. The technical implementations, experimental results and theoretical analyses are our own with scattered usage of AI for certain complex functions beyond our understanding and scope of this course.

# 1. Description of the Adopted Solution

## 1.1 Problem Context

This project is a Python/PySpark reimplementation and experimental evaluation of the element-wise Alternating Least Squares algorithm from:

> He, X., Zhang, H., Kan, M.-Y., & Chua, T.-S. (2016). *Fast Matrix Factorization for Online Recommendation with Implicit Feedback*.

The paper addresses two fundamental problems in recommender systems. First, **implicit feedback**, users rarely provide explicit ratings, hence, preferences must be inferred from observed interactions such as clicks or purchases. This creates the *one-class problem* where observed entries are positives, but missing entries are not confirmed negatives. Second, **online learning**, in real-world systems, new interactions arrive continuously and models must adapt in real-time without full retraining.

The paper also mentions that the algorithm is parallel, but provides no distributed implementation. Our task is to build a PySpark version using both RDDs and DataFrames, and to perform experimental analysis on one of the datasets from the paper. We opted to use the Yelp dataset because of its smaller size, allowing for easier usage in our computationally limited local environment.

## 1.2 Matrix Factorization for Implicit Feedback

Given a user--item interaction matrix $\mathbf{R} \in \mathbb{R}^{M \times N}$, matrix factorization maps each user $u$ and item $i$ into a shared $K$-dimensional latent space (equation numbers are from the original paper):
...
The paper replaces the standard uniform missing-data weight with a popularity-aware weighting scheme. The full objective as developed by the authors is:
...
Here $f_i = |R_i| / \sum_j |R_j|$ is normalized item popularity, $c_0$ is the overall missing-data weight scale (represented by `w0` in our code), and $\alpha$ controls popularity skewness. Setting $\alpha = 0$ recovers uniform weighting ($w_0 = c_0/N$).

## 1.4 Implementation Architecture

Our implementation has two layers:

**Layer 1 -- Spark (data pipeline)** PySpark DataFrames and RDDs handle all file reading, parsing, deduplication, grouping, and data transfer.

**Layer 2 -- NumPy (model training).** The eALS coordinate descent runs locally in Python (mainly NumPy) after interactions are collected from Spark. This was necessary for correctness, numerical stability, and debugging, and something that we accept is a limitation we could not overcome given our knowledge and understanding.

The dataset is from **Yelp** (`data/yelp.rating`) which is a file of tab-separated `(user_id, item_id, rating, timestamp)` records. Although explicit ratings are present in the raw file, we treat each row as an implicit positive interaction -- only the existence of a `(user, item)` pair is used and the rating value is discarded entirely.

## 1.5 Project File Structure

Also available as a repository on GitHub:

> *https://github.com/bhaveshchauhan2407/als-pyspark*

```
DBMS_PROJECT/
|-- data/
|   \-- yelp.rating                  # Raw Yelp interaction data
|-- figures/                         # Tests for this report
...
|-- requirements.txt
|-- run_alpha_sweep.py               # Experimental analysis
|-- run_experiments.py               # Grid search for baseline
|-- run_iteration_sweep.py           # Experimental analysis
|-- run_k_sweep.py                   # Experimental analysis
|-- run_scalability_sweep.py         # Experimental analysis
\-- run_w0_sweep.py                  # Experimental analysis
```
...
**Stage 2 -- Python skeleton.** A pure Python project skeleton was created before Spark was introduced: `config.py` (dataclass), `model.py` (stub), `train.py`, `evaluate.py` placeholders, and `data_loader.py` (pandas-based). This allowed the Java logic to be ported without complexity.
...
## 2.1 The eALS Update Rules (Algorithm 1 in the paper)
...
This cache is independent of $u$ and is pre-computed once per iteration, replacing $O(NK)$ per coordinate with a single $O(K)$ cache lookup.
...
## 2.2 Online Incremental Update (Algorithm 2 in the paper)

When a new user--item interaction $(u, i)$ arrives in the online system, the model does not retrain from scratch. Instead, it performs a minimal local update, if $u$ or $i$ have never been seen before (cold-start), their latent vectors are randomly initialized and appended to $\mathbf{P}$ and $\mathbf{Q}$ respectively, with the cache matrices extended accordingly. The new interaction is then registered with weight $w_{\text{new}} > 1$ (we use $w_{\text{new}} = 4.0$ as that appears to be the best performing value in the paper and it saved us effort on tuning), reflecting that fresh feedback is more indicative of current preference than historical training data. The model then runs one pass of the eALS coordinate descent, updating only $\mathbf{p}_u$ and $\mathbf{q}_i$ and every other user and item vector stays unchanged. After updating $\mathbf{p}_u$, the $\mathbf{S}^p$ cache is corrected with a rank-1 update, similarly after updating $\mathbf{q}_i$, the $\mathbf{S}^q$ cache is corrected. Since the update touches only the local neighbourhood of the new interaction, bounded by $|R_u|$ and $|R_i|$, the cost is entirely independent of the total number of users $M$, items $N$, or interactions $|R|$ in the system, making it suitable for real-time deployment.
...
The paper shows that eALS is parallel i.e. all user updates share only a read-only $\mathbf{S}^q$ cache and can proceed fully independently. In our implementation this maps to:
...
In our course implementation, steps 2--4 run locally in NumPy after `collect_rdd_interactions()` gathers data from Spark. This is a limitation but parallelism is preserved.

## 2.4 How We Utilised Spark DataFrames and RDDs
...
We use them for the full data ingestion and preprocessing pipeline.
...
**Grouping for preview.** `build_groupings_with_spark()` uses `groupBy("user_id").agg(F.collect_list("item_id"))` to produce a `user_id -> [item_ids]` summary DataFrame. This is used for inspection and logging in `main.py`.

**Impact on the algorithm.** The DataFrame layer ensures that deduplication, type-casting, and filtering happen at the Spark level before any data reaches the local Python model. This makes the pipeline robust and scalable and for larger datasets, Spark will distribute the file reading and deduplication across all available cores without any code changes.

### 2.4.2 Spark RDDs (`src/rdd_loader.py`)

We use them for explicit grouped interaction structures required by the model.
...
**Item grouping.** `build_item_users_rdd(interactions_rdd)` first calls `.map(lambda x: (x[1], x[0]))` to swap the key-value pair (making `item_id` the key), then `groupByKey().mapValues(list)` to produce `(item_id, [user_id, ...])`. The intermediate `map` is cheap (no shuffle) and only `groupByKey()` triggers a shuffle.
...
**Impact on the algorithm.** In a fully distributed version of eALS, the `build_user_items_rdd` structure would be the natural unit of parallelism meaning each partition of users could be assigned to a different worker for the update step but the `collect()` call prevents full distribution in our current implementation.
...
class FastALSConfig:
    ...
    show_loss: bool = True
    top_k: int = 10          # recommendation cutoff
    random_seed: int = 42
```

**Note:** `init_stdev = 0.001` was found necessary during Stage 3 to prevent divergence in early iterations. Larger values (e.g. `0.01`) caused the loss to oscillate rather than decrease monotonically.
...
def load_yelp_interactions_spark(spark, path: str, limit_rows: int = 5000):
    ...
    # Parsing each space separated line into 4 typed columns
    ...
    # Dropping rating/timestamp, deduplicate (user,item) pairs for offline 
    df = (
        df.select("user_id", "item_id")
          .dropna()
          .dropDuplicates()    # shuffle operation for implicit feedback
          .limit(limit_rows)
    )
    return df
```

**Note:** `dropDuplicates()` triggers a Spark shuffle but is essential, without it, a user with two reviews of the same business would generate duplicate training pairs, inflating that interaction's effective weight and biasing the model. The `limit_rows` parameter allows us to run experiments on data subsets of controlled size.
...
def build_groupings_with_spark(df):
    # groupBy is triggering a shuffle and collect_list is aggregating all item_ids per user
...
**Note:** These DataFrames are used for inspection and logging only. In `main.py` they are printed with `.show()` to demonstrate Spark's grouped aggregation capability.
...
def spark_df_to_rdd(df):
    # Converting structured DataFrame row to plain (user_id, item_id) tuple RDD
    return df.rdd.map(lambda row: (row["user_id"], row["item_id"]))

def build_user_items_rdd(interactions_rdd):
    # groupByKey is shuffling by user_id and mapValues is converting ResultIterable to list
    return interactions_rdd.groupByKey().mapValues(list)

def build_item_users_rdd(interactions_rdd):
    # Swapping key-value (cheap map) and then groupByKey (shuffle by item_id)
...
def collect_rdd_interactions(interactions_rdd) -> List[Tuple[int, int]]:
    # Bringing full RDD to driver, boundary between distributed and local
    return interactions_rdd.collect()
```

**Note:** `build_item_users_rdd` applies `.map(lambda x: (x[1], x[0]))` first to flip the tuple before `groupByKey()`. This is a narrow transformation (no shuffle). Only `groupByKey()` is wide (requires shuffle). This ordering correctly minimizes data movement.
...
**Note:** `SU` corresponds to $\mathbf{S}^p$ in the paper and `SV` corresponds to $\mathbf{S}^q$. Note the naming is from the Java source (`SU` for user matrix, `SV` for item matrix) -- the paper uses $S^p$ for users and $S^q$ for items. Both are correct.
...
def update_user(self, u: int) -> None:
    ...
    # Caching predictions and weights for this user's items
    for i in item_list:
    ...
    for f in range(self.config.factors):
        ...
        for i in item_list:
            # Removing f-th component from cached prediction
        ...
        # Restoring prediction after update
        for i in item_list:
            self.prediction_items[i] += self.U[u, f] * self.V[i, f]
    # Updating SU incrementally as rank-1 correction for changed row u
    for f in range(self.config.factors):
        for k in range(f + 1):
...
**Note:** The incremental `SU` update at the end is a correct rank-1 correction. This avoids a full $O(MK^2)$ recomputation after each user update, matching the Java implementation's efficiency. `update_item` applies the same pattern with $c_i$ weighting on both the cross-factor sum and the cache update, correctly implementing Eq. 13.
...
def update_model(self, raw_user_id, raw_item_id, w_new=4.0, online_iter=1):
    # Handling cold-start i.e. new user or item not seen in training
    ...
    # Registering the new interaction
    if i not in self.user_items[u]: self.user_items[u].append(i)
    if u not in self.item_users[i]: self.item_users[i].append(u)
    self.W[(u, i)] = w_new     # we are weighing new interaction higher than training
    # Updating locally only affected user and item vectors
    for _ in range(online_iter):
        self.update_user(u)
        self.update_item(i)
```

**Note:** Setting `w_new = 4.0` (four times the default training weight of 1.0) means new interactions have stronger immediate influence on the model, reflecting their higher recency value. This matches the spirit of the paper's Section 5.3.2, which finds optimal `w_new` around 4 for Yelp.
...
def run_one_iteration(model) -> None:
    # Updating all user vectors (SU updating incrementally inside update_user)
    for u in range(model.user_count):
        model.update_user(u)
    # Updating all item vectors (SV updating incrementally inside update_item)
...
**Note:** The evaluate-then-update ordering is critical i.e. metrics are computed before the model sees the new interaction, faithfully simulating a real online system that must make a recommendation without knowing the user's next action. `recommend_top_k` in `recommender.py` excludes training items from the candidate list using `seen_items = set(model.user_items[u])`.
...
**Note:** The incremental CSV write (`to_csv` inside the loop) is a design choice because if a long sweep is interrupted, the results up to that point are already saved. Each sweep runner (`run_k_sweep.py`, `run_alpha_sweep.py`, etc.) uses the same baseline parameter dictionary and varies only one key, ensuring controlled comparison.
...
All experiments use the **Yelp** dataset (`data/yelp.rating`). Each record is a `(user_id, item_id, rating, timestamp)` tuple; only the `(user_id, item_id)` pair is used as a positive implicit interaction. Experiments use subsets of 10,000 rows (offline: ~8,120 train, ~1,880 test | online: 9,000 train, 1,000 test stream) unless otherwise noted. We tried to be faithful to the original paper by performing the same experiments.

## 3.2 Evaluation Metrics

**Hit Ratio (HR@$K$)** measures whether the held-out item appears in the top-$K$ recommendations. HR is a recall-oriented metric meaning any correct hit counts equally regardless of its position within the list. We use $K=10$ throughout. Since the paper reports $K=100$, our values are not directly numerically comparable and our cutoff is stricter.

**Normalized Discounted Cumulative Gain (NDCG@$K$)** is rank-aware. A hit at rank 1 scores 1.0, at rank 2 scores $1/\log_2(3) \approx 0.631$, at rank $K$ scores $1/\log_2(K+1)$. NDCG penalizes hitting the correct item at a low rank, making it a precision-oriented complement to HR.
...
**Step 4 -- Choose a middle ground.** We sought a configuration that balanced good HR/NDCG with low loss and reasonable runtime. The row `K=12, max_iter=5, reg=0.1, w0=1.0, alpha=0.5` achieved HR=0.00532, NDCG=0.00215, loss=974.6, runtime~5.9s, therefore, sitting between the HR-maximizing $K=4$ configurations and the loss-minimizing $K=16$ configurations. This became the **baseline** for all one-parameter sweeps:
...
Offline HR is flat from $\alpha=0$ to $\alpha=0.75$, then drops sharply at $\alpha=1.0$. This suggests that for the offline protocol, the model saturates quickly and heavy popularity skewing at $\alpha=1.0$ actively hurts i.e. it over-penalizes popular items as negatives to the point where the model struggles to recommend any popular item. Offline NDCG declines monotonically with $\alpha$, indicating that while HR is robust to moderate $\alpha$ values, the ranking quality degrades as popularity weighting increases i.e. the correct item is being found but placed lower in the list.
...
\caption{Offline training runtime as dataset size grows.}
\end{figure}
...
Offline training runtime scales from 6.0s at 10k rows to 37.3s at 100k rows, approximately a $6\times$ increase for a $10\times$ increase in data size. The overall scaling is consistent with the theoretical $O((M+N)K^2 + |R|K)$ complexity.

Online runtime grows much faster: 7.5s at 10k to 129s at 100k, approximately $17\times$ for $10\times$ data. This super-linear growth is due to an important numerical issue, the online loss at 50k and 100k rows (1.44M and 8.33M respectively, compared to ~990 at 10k) indicates numerical divergence in the online update protocol at larger scales. This appears to be a limitation of purely incremental updates and an important finding for the scalability section.
...
**Parallel structure.** User updates depend only on the read-only $\mathbf{S}^q$ cache. Item updates depend only on the read-only $\mathbf{S}^p$ cache. This maps naturally to Spark's partition-based execution where each partition of users can be updated independently, with the shared cache broadcast to all workers.
...
**Online instability at scale.** A very significant finding, online loss diverges at 50k and 100k rows (reaching 1.44M and 8.33M respectively). A production system would require periodic full cache refresh.

**Online protocol sensitive to offline training depth.** The iteration sweep reveals that 2--3 offline training iterations give the best online performance, while loss-minimizing configurations (15--20 iterations) cause online metrics to collapse. This tension between offline loss and online adaptability is not discussed in the paper and is a cause for confusion for us.
...
- Metrics at HR@10 / NDCG@10 rather than paper's HR@100 / NDCG@100, hence, our values are not numerically comparable.
...
These follow the exact identical structure as `run_k_sweep.py`, varying `sweep_name` and `sweep_values`, please refer to our GitHub repository to view the full code:
...
# Please see the full file on our GitHub repository for make_k_charts(), make_w0_charts(), make_alpha_charts(),
...
## A.19 Repository on GitHub

To view all of our code, experimental results, a README and the markdown for this report, please refer to our repository for this project on GitHub:
